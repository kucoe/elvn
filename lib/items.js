...
exports.Items.prototype = {
    ...
    runItem: function (item, onTime) {
        ...
        this.checkInit();
        if (!item.planned) {
            this.saveItem({id: item.id, plan: item.plan, text: item.text, planned: true, completedOn: null});
        }
        var self = this;
        this.getTimer().runElvn(item, function (completed) {
            self.saveItem({id: item.id, color: item.color, text: item.text, planned: true, completedOn: new Date()});
            if (onTime) {
                onTime(completed);
            }
        });
