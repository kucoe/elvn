...
Command.prototype.exec = function (display, config) {
    if(!display || !config) {
        return;
    }
    var current = this.run(display, config);
    var next = this.next;
    if (next) {
        ...
        tasks.forEach(function (task, i) {
            if (c) {
                if (c == '>') {
                    ...
                    config.saveTask({id: task.id, color: c, text: task.text, planned: task.planned, completedOn: task.completedOn});
                    next = new SwitchList(c);
                } else {
                    display.show(this.help(c));
                }
            } else if (!this.color && !this.text) {
                next = null;
...
exports.init = function (cli, display, config) {
    ...
    cli.command('{color}:{text}', 'add task or idea', {color: '[roygbfp]?', text: '.+'}, function (input, args) {
        var cmd = new EditTask(args.color, args.text);
        cmd.exec(display, config);
        return cmd;
    });
    cli.command('\\\\s', 'toggle silent state for timer', function() {
        config && config.getTimer().toggleSilent();
    });
};
