...
exports.init = function (cli, display, items) {
    cli.command('/{journal}{date}', 'show items by plan',
        {journal: '@|journal', date: '( \\d{1,2}((-\\d{1,2})|)((-20\\d{2})|))|'}, function (input, args) {
            ...
            if (date) {
                ...
                if (!date) {
                    display.show('Wrong date format, expected MM-dd-yyyy');
                    return null;
                }
            }
    ...
    cli.command('@{time}-{entry}', 'add journal entry', {time: '(\\d{1,2}((:\\d{1,2})|)(a|p|A|P)m)|', entry: '.+'},
        function (input, args) {
            var time = args.time;
            if (time) {
                if (!validateTime(time)) {
                    display.show('Wrong time format, expected hh:mm a');
                    return null;
                }
            }
            var cmd = new JournalTask(args.time, args.entry);
            cmd.exec(display, items);
            return cmd;
        });
