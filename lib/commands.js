        ...
        if (c) {
            if (c === ':') {
                ...
                if (timer.running) {
                    ...
                    var tasks = items.getList('t').tasks;
                    if (tasks.length > 0) {
                        var onTime = function () {
                            var task = tasks.shift();
                            if (task) {
                                items.runItem(task, onTime);
                            }
                        };
                        onTime();
                    }
...
var EditTask = function (plan, text) {
    ...
    this.updateTask = function (item, items, currentList) {
        ...
        var planned = item.planned || it.is(it.today, currentList);
        var completedOn = item.completedOn || null;
        it.set(c, it.today[0], planned);
        it.set(c, it.done[0], completedOn != null);
    ...
    this.run = function (display, items) {
        ...
        its.forEach(function (item, i) {
            ...
            if (c) {
                ...
                if (c == '>') {
                    first = item;
                    p = it.set(p, it.today[0], true);
                    p = it.set(p, it.done[0], false);
                    next = null;
                } else if (c == 'x') {
                    ...
                    if (this.plan && this.plan.length > 0) {
                        ...
                        p = it.set(p, it.today[0], false);
                        p = it.set(p, it.done[0], false);
                    }
                } else if (c == 'v') {
                    p = it.set(p, it.done[0], true);
                } else if (c == '^') {
                    p = it.set(p, it.done[0], false);
                } else if (c == ':') {
                    p = this.plan;
