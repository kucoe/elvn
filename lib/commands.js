var util = require('util');

var Command = function () {
    this.forwardEnabled = true;
};

Command.prototype.run = function (display, config) {
    return null;
};

Command.prototype.forward = function (result, display, config) {
    return result.run(display, config);
};

var SwitchIdeas = function () {
    Command.call(this);
    this.run = function (display, config) {
        var ideas = config.getIdeas();
        display.showIdeas(ideas);
        return '@';
    };
};

var SwitchList = function (color) {
    Command.call(this);
    this.color = color;
    this.run = function (display, config) {
        var list = config.getList(this.color);
        if (list != null) {
            display.showList(list);
            return list.getColor();
        }
        return null;
    };
};

var SwitchListEdit = function () {
    Command.call(this);
    this.run = function (display, config) {
        display.showLists(config);
        return '&';
    };
};

var SwitchStatus = function () {
    Command.call(this);
    this.run = function (display, config) {
        display.showStatus(config.getStatus());
        return null;
    };
};

var SwitchSync = function (command) {
    Command.call(this);
    this.command = command;
    this.run = function (display, config) {
        var sync = config.getSync();
        var c = this.command;
        if (c) {
            if (sync != null) {
                if (c === '<') {
                    sync.pull();
                } else if (c === '>') {
                    sync.push();
                } else if (c === '-') {
                    sync.restore();
                } else {
                    display.showHelp(this.help(c));
                }
            } else {
                display.showHelp("Sync is not initialized");
            }
        }
        config.reload();
        display.showConfig(config.getConfig());
        return '!';
    };
    this.help = function (invalid) {
        return "\tWrong sync command: " + invalid + "\n" + "\tAvailable commands:\n" + "\t> push\n" + "\t< pull\n"
            + "\t- revert";
    };
};

var SwitchTimer = function (command) {
    Command.call(this);
    this.command = command;
    this.run = function (display, config) {
        var reshow = true;
        var c = this.command;
        if (c == null) {
            Timer.show();
            return null;
        } else {
            if (c === ':') {
                Timer.pause();
            } else if (c === '>') {
                if (Timer.isRunning()) {
                    Timer.resume();
                } else {
                    var tasks = config.getList(ListColor.Today).getTasks();
                    if (tasks.length > 0) {
                        var onTime = function () {
                            var task = tasks.shift();
                            if (task) {
                                config.runTask(task, onTime);
                            }
                        };
                        onTime();
                    }
                    reshow = false;
                }
            } else if (c === 'x') {
                Timer.cancel();
            } else if (c === '^') {
                Timer.fire(false);
            } else if (c === 'v') {
                Timer.fire(true);
            } else {
                display.showHelp(this.help(c));
            }
            if (reshow) {
                Timer.show();
            }
        }
        return null;
    };
    this.help = function (invalid) {
        return "\tWrong timer command: " + invalid + "\n" + "\tAvailable commands:\n" + "\t> run/resume task\n"
            + "\t: pause task\n" + "\t^ skip current stage\n" + "\tx cancel task\n" + "\tv complete task";
    };
};

var EditTask = function (color, text) {
    this.color = color;
    this.text = text;
    this.execute = function (display, config) {
        var currentList = display.getCurrentList();
        if (currentList == '@') {
            var t = this.text;
            if (t == null) {
                t = "";
            }
            if (this.color) {
                t = this.color + ":" + t;
            }
            config.saveIdea(new Idea(new Date().getTime(), t.trim()));
            return this.forward(new SwitchIdeas(), display, config);
        }
        if (currentList == '&') {
            config.saveList(list, text);
            return this.forward(new SwitchListEdit(), display, config);
        }
        this.updateTask(new Task(null, Blue.toString(), text, false, null), config, currentList);
        return this.forward(new SwitchList(currentList), display, config);
    };
    this.updateTask = function (task, config, currentList) {
        var id = task.getId();
        if (id == null) {
            id = new Date().getTime();
        }
        var c = this.color;
        var t = this.text;
        if (c == null || ListColor.color(c) == null || this.isRestrictedList(c)) {
            if (c != null && ListColor.color(c) == null) {
                t = t == null ? c + ":" : c + ":" + t;
            }
            if (!this.isRestrictedList(currentList)) {
                c = currentList;
            } else {
                c = task.getList();
            }
        }
        if (t != null) {
            t = this.processText(task.getText(), t);
        } else {
            t = task.getText();
        }
        var planned = task.isPlanned() || Today.equals(ListColor.color(currentList));
        var completedOn = task.getCompletedOn();

        var update = new Task(id, c, t, planned, completedOn);
        config.saveTask(update);
    };
    this.processText = function (oldText, newText) {
        var t = newText;
        if (t.indexOf("%") != -1) {
            var split = t.split("%");
            var replace = "";
            if (split.length > 1) {
                replace = split[1];
            }
            t = oldText.replace(split[0], replace);
        } else if (t.charAt(0) == "+") {
            t = oldText.concat(t.substring(1));
        } else if (t.charAt(0) == "-") {
            t = oldText.replace(t.substring(1), "");
        }
        return t.trim();
    };
    this.isRestrictedList = function (color) {
        var c = ListColor.color(color);
        return ListColor.isSystemColor(c);
    };
};

var SearchTask = function (text, query) {
    this.query = query;
    this.execute = function (display, config) {
        var currentList = display.getCurrentList();
        if (currentList == '@') {
            var ideas = config.findIdeas(this.query);
            if (this.text) {
                ideas.forEach(function (idea) {
                    config.saveIdea(new Idea(idea.getId(), this.processText(idea.getText(), this.text)));
                }, this);
            }
            ideas = config.findIdeas(this.query);
            display.showIdeas(ideas);
            return currentList;
        }
        var tasks = config.findTasks(this.query);
        if (this.text || this.color) {
            tasks.forEach(function (task) {
                this.updateTask(task, config, currentList);
            });
        }
        var result = [];
        tasks.forEach(function (task) {
            result.push(config.getById(task.getId()));
        });
        display.showTasks(result);
        return ListColor.All.toString();
    };
    EditTask.call(this, '', text);
};

var LocateTask = function (color, text, positions, command) {
    this.positions = positions;
    this.command = command;
    this.execute = function (display, config) {
        var currentList = display.getCurrentList();
        var c = this.command;
        var next = new SwitchIdeas();
        if (currentList == '@') {
            var ideas = this.getIdeas(config);
            ideas.forEach(function (idea, i) {
                if (c) {
                    if (c == 'x') {
                        config.removeIdea(idea);
                    } else if (ListColor.color(c) != null) {
                        config.removeIdea(idea);
                        config.saveTask(new Task(idea.getId(), c, idea.getText(), false, null));
                        next = new SwitchList(ListColor.All.toString());
                    }
                } else if (this.text == null && this.color == null) {
                    next = null;
                    display.showIdea(idea, this.positions[i]);
                } else if (this.text) {
                    var t = this.text;
                    if (t == null) {
                        t = "";
                    }
                    t = this.color == null ? t : this.color + ":" + t;
                    config.saveIdea(new Idea(idea.getId(), this.processText(idea.getText(), t)));
                }
            }, this);
            return next ? this.forward(next, display, config) : currentList;
        }
        var tasks = this.getTasks(currentList, config);
        next = new SwitchList(currentList);
        var first = null;
        tasks.forEach(function (task, i) {
            if (c) {
                if (c == '>') {
                    first = task;
                    config.saveTask(new Task(task.getId(), task.getList(), task.getText(), true, null));
                    next = null;
                } else if (c == 'x') {
                    config.removeTask(task);
                } else if (c == '+') {
                    config.saveTask(new Task(task.getId(), task.getList(), task.getText(), true, null));
                } else if (c == '-') {
                    config.saveTask(new Task(task.getId(), task.getList(), task.getText(), false, null));
                } else if (c == 'v') {
                    config.saveTask(new Task(task.getId(), task.getList(), task.getText(), task.isPlanned(), new Date()));
                } else if (c == '^') {
                    config.removeTask(task);
                    config.saveTask(new Task(task.getId(), task.getList(), task.getText(), task.isPlanned(), null));
                } else if (c == '@') {
                    config.removeTask(task);
                    config.saveIdea(new Idea(task.getId(), task.getText()));
                    next = new SwitchIdeas();
                } else {
                    display.showHelp(this.help(c));
                }
            } else if (this.color == null && this.text == null) {
                next = null;
                display.showTask(task, this.positions[i]);
            } else {
                this.updateTask(task, config, currentList);
            }
        }, this);
        if (first) {
            config.runTask(first, null);
        }
        return next ? this.forward(next, display, config) : currentList;
    };
    this.getIdeas = function (config) {
        var ideas = config.getIdeas();
        if (this.positions.length == 0) {
            return ideas;
        }
        var res = [];
        this.positions.forEach(function (pos) {
            if (ideas.length >= pos) {
                res.push(ideas[pos - 1]);
            }
        });
        return res;
    };
    this.getTasks = function (currentList, config) {
        var res = [];
        if (currentList == '@') {
            return res;
        }
        var list = config.getList(ListColor.color(currentList));
        if (list) {
            var tasks = list.getTasks();
            if (this.positions.length == 0) {
                return tasks;
            }
            this.positions.forEach(function (pos) {
                if (tasks.length >= pos) {
                    res.push(tasks[pos - 1]);
                }
            });
        }
        return res;
    };
    this.help = function (invalid) {
        return "\tWrong task command: " + command + "\n" + "\tAvailable commands:\n" + "\t> run task\n"
            + "\t@ convert task to idea\n" + "\t+ make task planned\n" + "\t- make task not planned\n"
            + "\tv make task completed\n" + "\t^ make task not completed\n" + "\tx delete task";
    };
    EditTask.call(this, color, text);
};

util.inherits(SwitchIdeas, Command);
util.inherits(SwitchList, Command);
util.inherits(SwitchListEdit, Command);
util.inherits(SwitchStatus, Command);
util.inherits(SwitchSync, Command);
util.inherits(SwitchTimer, Command);
util.inherits(EditTask, Command);
util.inherits(SearchTask, EditTask);
util.inherits(LocateTask, EditTask);


exports.Command = Command;
exports.SwitchIdeas = SwitchIdeas;
exports.SwitchList = SwitchList;
exports.SwitchListEdit = SwitchListEdit;
exports.SwitchStatus = SwitchStatus;
exports.SwitchSync = SwitchSync;
exports.SwitchTimer = SwitchTimer;
exports.EditTask = EditTask;
exports.SearchTask = SearchTask;
exports.LocateTask = LocateTask;


exports.init = function (cli) {
    cli.command('/{color}', 'show tasks list by color or label', {color: '\\w+'}, function (input, args) {
        return new SwitchList(args.color);
    });
    cli.command('@', 'show ideas', function () {
        return new SwitchIdeas();
    });
    cli.command('&', 'show available lists', function () {
        return new SwitchListEdit();
    });
    cli.command('!', 'show status', function () {
        return new SwitchStatus();
    });
    cli.command('%{command}', 'sync config and executes command if specified.' +
        '\n\tSync commands:\n\t> push\n\t< pull\n\t- revert', {command: '[-<>]?'}, function (input, args) {
        return new SwitchSync(args.command);
    });
    cli.command('${command}', 'show timer and executes command if specified. ' +
        '\n\tTimer commands:\n\t> run/resume task\n\t: pause task\n\t^ skip current stage\n\tx cancel task\n\tv complete task',
        {command: '[:>^vx]?'}, function (input, args) {
            return new SwitchTimer(args.command);
        });
    cli.command('?{query}={search}%{replacement}', 'search task or idea and replace search with replacement', {query: '\\w+', search: '\\w+', replacement: '\\w+'}, function (input, args) {
        return new SearchTask(args.search + '%' + args.replacement, args.query);
    });
    cli.command('?{query}%{replacement}', 'search task or idea and replace query with replacement', {query: '\\w+', replacement: '\\w+'}, function (input, args) {
        return new SearchTask(args.query + '%' + args.replacement, args.query);
    });
    cli.command('?{query}={text}', 'search task or idea and change it text', {query: '\\w+', text: '.+'}, function (input, args) {
        return new SearchTask(args.text, args.query);
    });
    cli.command('?{query}', 'search task or idea', {query: '\\w+'}, function (input, args) {
        return new SearchTask('', args.query);
    });
    cli.command('#{start}-{end}{command}', 'show task or idea by range (ex. #1-4) and executes command if specified. ' +
        '\n\tTask commands:\n\t> run task\n\t@ convert task to idea\n\t+ make task planned\n\t- make task not planned\n' +
        '\tv make task completed\n\t^ make task not completed\n\tx delete task',
        {start: '\\d+', end: '\\d+', command: '[>@+-v^x]?'}, function (input, args) {
            var start = args.start;
            var end = args.end;
            if (start > end) {
                var tmp = start;
                start = end;
                end = tmp;
            }
            var positions = [];
            var i = 0;
            while (start <= end) {
                positions[i++] = start++;
            }
            return new LocateTask('', '', positions, args.command);
        });
    cli.command('#{positions}{command}', 'show task or idea by positions (ex. #2 or #2,4) and executes command if specified.' +
        '\n\tTask commands:\n\t> run task\n\t@ convert task to idea\n\t+ make task planned\n\t- make task not planned\n' +
        '\tv make task completed\n\t^ make task not completed\n\tx delete task',
        {positions: '(\\d+,?)+', command: '[>@+-v^x]?'}, function (input, args) {
            var positions = [];
            if (typeof args.positions === 'number') {
                positions = [args.positions];
            } else {
                var split = args.positions.split(',');
                split.forEach(function (pos) {
                    positions.push(parseInt(pos));
                });
            }
            return new LocateTask('', '', positions, args.command);
        });
    cli.command('{color}:{text}', 'add task or idea', {color: '[roygbfp]?', text: '\\w+'}, function (input, args) {
        return new EditTask(args.color, args.text);
    });
};
