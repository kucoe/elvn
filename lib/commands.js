...
var EditTask = function (color, text) {
    ...
    this.run = function (display, config) {
        ...
        if (currentList == '@') {
            ...
            config.saveIdea({id: new Date().getTime(), text: t.trim()});
    ...
    this.updateTask = function (task, config, currentList) {
        ...
        var c = this.color;
        var t = this.text;
        if (c == null || !colors.is(colors.colors, c) || colors.is(colors.restricted, c)) {
            if (c != null && !colors.is(colors.colors, c)) {
                t = t == null ? c + ":" : c + ":" + t;
            }
        ...
        var completedOn = task.completedOn;
