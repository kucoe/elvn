...
exports.Display.prototype = {
    ...
    formatItem: function (currentList, item, pos) {
        ...
        if (items.is(items.today, currentList)) {
            items.set(plan, items.today[0], false);
            items.set(plan, items.today[1], false);
        } else if (items.is(items.all, currentList)) {
            items.set(plan, items.all[0], false);
            items.set(plan, items.all[1], false);
        } else if (items.is(items.done, currentList)) {
            items.set(plan, items.done[0], false);
            items.set(plan, items.done[1], false);
        }
        items.set(plan, currentList, false);
        if(plan.length > 0) {
            res += plan + ':';
        }
        res += item.text;
